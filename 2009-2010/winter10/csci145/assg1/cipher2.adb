...
BEGIN --Cipher
   ...
   -- User supplies message file, output file name, and cipher key
   ...
   -- Open Input file and create new output file for deciphered message
   ...
   -- Loop through lines by character and looks for letters. This also
   -- uses a different reference point based on whether the letter is
   -- uppercase or lowercase. It ignores & passes along anything that
   -- is not a letter.
   ...
   -- Output confirmation and farewell
   Ada.Text_IO.New_Line;
   Ada.Text_IO.Put (Item => "--------------------------------------");
   Ada.Text_IO.New_Line;
